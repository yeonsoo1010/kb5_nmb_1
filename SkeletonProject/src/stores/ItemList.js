...
export const use_money_list_store = defineStore("money_list", () => {
    ...
    const totalIncome = computed(() => {
        return state.money_list.filter((item) => state.categories.find((cat) => cat.id === item.category_id)?.type === "income").reduce((sum, item) => sum + item.price, 0);
    });

    const totalExpense = computed(() => {
        return state.money_list.filter((item) => state.categories.find((cat) => cat.id === item.category_id)?.type === "outcome").reduce((sum, item) => sum + item.price, 0);
    });
